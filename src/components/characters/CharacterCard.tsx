  ...
  return (
    <Card hover padding="sm" onClick={onClick} className={clsx(selected && 'ring-2 ring-primary-500')}>
      <div className="flex items-center gap-2">
        <div className="w-10 h-10 rounded-lg bg-[rgb(var(--color-bg))] overflow-hidden flex-shrink-0 border border-[rgb(var(--color-border))]">
          {avatar ? (
            <img src={avatar} alt={character.name} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-primary-400 text-sm font-bold">
              {character.name[0]}
            </div>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {character.gender && <span className="text-[10px] text-[rgb(var(--color-text-secondary))] flex-shrink-0">{character.gender}</span>}
            {character.age && <span className="text-[10px] text-[rgb(var(--color-text-secondary))] flex-shrink-0">{character.age}</span>}
          </div>
          {tags.length > 0 && (
