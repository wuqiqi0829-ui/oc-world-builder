...
export default function Modal({ open, onClose, title, children, footer, maxWidth = 'max-w-md', maxHeight = 'max-h-[80vh]' }: ModalProps) {
  if (!open) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
      <div className={`relative bg-[rgb(var(--color-surface))] rounded-card shadow-xl w-full ${maxWidth} ${maxHeight} flex flex-col`}>
