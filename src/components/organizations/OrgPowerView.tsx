...
function SortableEntryCard({ entry, onPreview, showHandle }: { entry: CustomEntry; onPreview: () => void; showHandle: boolean }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={clsx('relative group/card break-inside-avoid', isDragging && 'z-10 opacity-90')}>
      ...
      <Card hover padding="sm" onClick={onPreview} className="!p-0 overflow-hidden flex flex-col">
        <div className="aspect-video bg-[rgb(var(--color-bg))] flex items-center justify-center overflow-hidden rounded-t-card">
          {imgSrc ? (
            <img src={imgSrc} alt={entry.name} className="w-full h-full object-cover" />
          ) : (
            <span className="text-3xl font-bold text-primary-300">{entry.name.charAt(0)}</span>
          )}
        </div>
        <div className="p-2 sm:p-3 text-center flex-1 flex flex-col items-center justify-center">
          <h4 className="text-base font-semibold">{entry.name}</h4>
          {brief && (
            <p className="text-sm text-[rgb(var(--color-text-secondary))] mt-1 whitespace-pre-wrap">「{brief}」</p>
          )}
        </div>
      </Card>
    </div>
...
export default function OrgPowerView({ worldId, onCreateEntry, onPreviewEntry }: Props) {
  const { categories, entries, fetchCategories, fetchEntries } = useCategories();
  const [tab, setTab] = useState<'组织' | '势力'>('组织');
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-1 bg-[rgb(var(--color-bg))] rounded-lg p-0.5 border border-[rgb(var(--color-border))]">
          <button
            className={clsx('text-xs px-3 py-1.5 rounded-md transition-colors', tab === '组织' ? 'bg-white dark:bg-gray-700 shadow-sm font-medium' : 'text-[rgb(var(--color-text-secondary))] hover:text-[rgb(var(--color-text))]')}
            onClick={() => setTab('组织')}
          >组织</button>
          <button
            className={clsx('text-xs px-3 py-1.5 rounded-md transition-colors', tab === '势力' ? 'bg-white dark:bg-gray-700 shadow-sm font-medium' : 'text-[rgb(var(--color-text-secondary))] hover:text-[rgb(var(--color-text))]')}
            onClick={() => setTab('势力')}
          >势力</button>
        </div>
